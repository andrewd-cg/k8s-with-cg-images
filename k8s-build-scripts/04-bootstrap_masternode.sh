...
# Get list of images needed to install
kubeadm config images list --image-repository $IMAGE_REPO

### Run kubeadm init on master node to bootstrap cluster (ignore swap error since it's been turned off but not rebooted)
kubeadm init --image-repository $IMAGE_REPO --apiserver-advertise-address=$MASTER_NODE_IP_ADDR  --apiserver-cert-extra-sans=$MASTER_NODE_IP_ADDR  --pod-network-cidr=$POD_CIDR --node-name $NODENAME --ignore-preflight-errors Swap
